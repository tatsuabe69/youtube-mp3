...
def get_ffmpeg_dir() -> str | None:
    import sys, platform
    log_path = Path(tempfile.gettempdir()) / "yt-mp3-ffmpeg-debug.txt"
    lines = []
    if getattr(sys, 'frozen', False):
        base = Path(sys._MEIPASS)
        lines.append(f"frozen=True, _MEIPASS={base}")
        try:
            entries = list(base.rglob('ffmpeg*'))
            lines.append(f"ffmpeg* files in _MEIPASS: {entries}")
        except Exception as e:
            lines.append(f"rglob error: {e}")
        ffmpeg = base / ('ffmpeg.exe' if platform.system() == 'Windows' else 'ffmpeg')
        lines.append(f"looking for: {ffmpeg}, exists={ffmpeg.exists()}")
        log_path.write_text('\n'.join(lines))
        if ffmpeg.exists():
            return str(base)
        # rglob フォールバック
        ...
        for f in base.rglob(ffmpeg_name):
            if (f.parent / ffprobe_name).exists():
                lines.append(f"found via rglob: {f.parent}")
                log_path.write_text('\n'.join(lines))
                return str(f.parent)
        lines.append("not found in bundle")
        log_path.write_text('\n'.join(lines))
        return None
    try:
